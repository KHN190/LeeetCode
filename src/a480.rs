...
fn run_test() {
    ...
    assert_eq!(median_sliding_window(nums.clone(), 3), vec!(2.0, 3.0, 4.0, 4.0, 2.0));
    assert_eq!(median_sliding_window(nums.clone(), 4), vec!(2.5, 3.5, 3.5, 3.0));

    // test delete only once
    let nums = vec!(1,2,3,4,2,3,1,4,2);
    assert_eq!(median_sliding_window(nums.clone(), 3), vec!(2.0, 3.0, 3.0, 3.0, 2.0, 3.0, 2.0));
